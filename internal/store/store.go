...
type Store interface {
	...
	// (UpdateUpstream runs with no transaction, so there is no second
	// statement for a crash to land between). resetTest appends
	// `last_test_at=NULL, last_test_ok=NULL` — the caller passes ResetTest when
	// url, transport, auth_type or auth_config changed — so a dot never vouches
	// for a configuration nobody tested; the two columns are never written from
	// the struct. writeAuth adds auth_config to the SET list — the caller passes
	// WriteAuth only when the request carried a credential — so an edit that
	// did not touch the credential never rewrites the ciphertext it read.
	...
	// dropped rather than vouching for settings it never tested. The compare is
	// on the canonical string fmtTime(seen): every writer of updated_at is fmtTime
	// and the column is TEXT on both drivers, so the reformat reproduces the stored
	...
	// updated_at is not bumped: a test is not an edit. Returns ErrNotFound when
	// no row matched — deleted, or edited since seen. Between two overlapping
	// tests of one unchanged row the later write wins, deliberately.
