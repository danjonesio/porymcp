...
- The Postgres profile is actually wired: `DATABASE_URL` is overridable,
  ...
  SQLite→Postgres migration. Needs a recent Compose (`depends_on.required`,
...
One underscore was not just untidy. Two members `s` and `s_x` each exporting a
...
supports put the secret in an ordinary header (`api_key` → `X-API-Key` by
...
catalogue request is skipped, so its tools are simply missing from the merged
...
- The dashboard can ask an upstream what tools it offers — from the Add upstream
  ...
  and four at once. Both routes are new — but note that this is an outbound
