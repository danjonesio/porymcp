...
- `make vuln` runs `govulncheck` at a pinned version, and `make web-audit` runs `npm audit` over the whole lockfile, dev and optional platform packages included, which is more than any one machine installs and is the command someone evaluating PoryMCP runs. Both read a vulnerability database that is fetched fresh, so either can fail on a commit that changed nothing. That is the point; the fix is a dependency bump. The audit fails on a high or critical advisory; moderate and low advisories print in the log and pass. The `govulncheck` version is pinned in the `Makefile` and moves by hand, because no Dependabot lane reads a `go run` argument: change the version, run `make vuln`, and treat a new finding as a finding.
