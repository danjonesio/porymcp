...
- The Go toolchain that compiles is the `toolchain` line in `go.mod`, not whatever `go version` prints; the first `go` command downloads it when they differ. Offline builders can set `GOTOOLCHAIN=local` (see the README).
