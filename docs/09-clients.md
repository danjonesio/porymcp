...
One combined server instead — one server, the merged catalogue:
...
One combined server instead — one `mcpServers` entry pointing at
...
One combined server instead — one `[mcp_servers.my_agent]` table pointing at
`http://localhost:8080/{virtual_key_id}/mcp`.
...
One combined server instead — one `mcp` entry pointing at
`http://localhost:8080/{virtual_key_id}/mcp`.
...
One combined server instead — one `mcpServers` entry whose `httpUrl` is
`http://localhost:8080/{virtual_key_id}/mcp`.
