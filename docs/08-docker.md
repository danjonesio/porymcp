...
- Subcommands: `/porymcp healthcheck` (above) and `/porymcp rekey`, which re-encrypts every stored credential under the current `ENCRYPTION_KEY` after a rotation (`docs/11-deployment.md` §12). Any other first argument prints a usage line and exits `2` — a mistyped `rekey` never starts a second server on the live database.
...
- `PUBLIC_URL` — used when minting `proxy_url` and every entry in a virtual key's `endpoints[]`, and as the expected host/scheme for proxy-endpoint checks. Default `http://localhost:8080` (no scheme enforcement out of the box).
