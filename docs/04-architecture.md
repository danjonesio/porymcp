...
- **Language**: Go (preferred – single static binary, low resource)  
  Alternative: TypeScript (Fastify/Hono) if you want faster UI iteration
- **DB**: SQLite (default, zero-config) + optional Postgres
- **HTTP router**: chi or gin (Go) / Hono or Fastify (TS)
- **UI**: React + Tailwind CSS + Headless UI (PoryMCP's own component set in `web/src/components/`)
- **Auth**: High-entropy API keys (hashed with argon2id/bcrypt)
- **Secrets**: Encrypted at rest (AES-256-GCM)
- **Observability**: Structured JSON logs + optional Prometheus
