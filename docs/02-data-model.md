...
- `slug` (string) – stable, unique, URL- and tool-name-safe identifier
  (`[a-z0-9]`, `_` and `-` inside, 1–40 chars, no repeated separator, not
...
- `url` (string) – the real MCP endpoint, and it must be the **final** one: the
  ...
  can actually dial; anything else is `400`. Whether that host *should* be
...
- `last_test_ok` (nullable) – whether that run completed the handshake **and**
  ...
  (the cause of the second is `auth_status` on the same row — `docs/03-api.md`).
...
back), and by `rekey` — nothing else. `auth_type: none` rows may hold a blob too
...
- `tool_allowlist` / `tool_denylist` (optional) – per-key overrides applied to
  every target. Precedence is **key denylist → key allowlist → group
  ...
  blocks a prompt actually named `docs__search`, a bare entry blocks the prompt
...
- **Deny rules only are rewritten, and only by adding.** An unscoped entry that
  ...
  member gains `{member_slug}__{entry}` for every member, because it may simply
