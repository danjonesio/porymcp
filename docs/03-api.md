...
| upstream `enabled` | set | `400 enabled must be true or false` | — |
...
| key `rate_limit` | set; `0` (like `null`) means unlimited | **cleared** — unlimited | — |
...
URL: a `user:password@` and a query string are never written into one — which
...
`PATCH` validates only a list the request actually sends, and against the target
...
means. When `target_type`/`target_id` actually change, the resulting
...
- `GET /stats` — dashboard counters (active virtual keys, calls today, error
  ...
  configured key opens the stored credential — drives the Overview notice),
  ...
  `ENCRYPTION_KEY_PREVIOUS` key — a rotation `porymcp rekey` has not finished;
...
provokes — it sends no conditional requests), a relative one, one that does not
...
writes no audit row: the member is skipped, its tools are simply absent from the
